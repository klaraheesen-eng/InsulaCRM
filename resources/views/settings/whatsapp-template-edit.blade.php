...
@section('title', __('Edit WhatsApp Template'))
@section('page-title', __('Edit WhatsApp Template'))

@section('breadcrumbs')
<li class="breadcrumb-item"><a href="{{ route('settings.index') }}">{{ __('Settings') }}</a></li>
<li class="breadcrumb-item"><a href="{{ route('whatsapp-templates.index') }}">{{ __('WhatsApp Templates') }}</a></li>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-actions">
                    <a href="{{ route('whatsapp-templates.index') }}" class="btn btn-ghost-secondary btn-sm">{{ __('Back') }}</a>
                </div>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('whatsapp-templates.update', $template->id) }}">
                    @csrf @method('PUT')
                    <div class="mb-3">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <a href="{{ route('whatsapp-templates.preview', $template->id) }}" class="btn btn-outline-secondary" target="_blank">{{ __('Preview') }}</a>
                    </div>
                </form>
            </div>
        </div>
